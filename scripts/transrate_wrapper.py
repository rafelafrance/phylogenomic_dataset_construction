...
def transrate_ref(assembly, pe_fq1, pe_fq2, num_cores, DIR, reference):
    ...
    path_assembly, file_assembly = (os.path.split(assembly))  # splits the
...
if __name__ == "__main__":
    if len(sys.argv) == 6:
        ...
        transrate_ref(assembly=sys.argv[1], pe_fq1=sys.argv[2],
                      pe_fq2=sys.argv[3], num_cores=int(sys.argv[4]),
                      DIR=sys.argv[5], reference=sys.argv[6])
    else:
        print("Usage:")
        print(
            "python transrate_wrapper.py assembly_fasta fastq_filtered_pe_reads1 fastq_filtered_pe_reads2 num_cores output_dir reference(optional) ")
        sys.exit(0)
