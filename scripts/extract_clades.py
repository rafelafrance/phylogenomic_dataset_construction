...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print(
            "python extract_clades.py inDIR treefileending outDIR "
            "MIN_INGROUP_TAXA TAXON_CODE phypartsDIR")
        sys.exit(0)
    ...
    for treefile in os.listdir(inDIR):
        if not treefile.endswith(treefileending):
            continue
        ...
        inclades = extract_rooted_ingroup_clades(curroot, INGROUPS, OUTGROUPS,
                                                 MIN_INGROUP_TAXA)
        inclade_count = 0
        for inclade in inclades:
            inclade_count += 1
            with open(outDIR + treefile + "." + str(inclade_count),
                      "w") as outfile:
                outfile.write(newick3.tostring(inclade) + ";\n")
            for node in inclade.iternodes():
                if node.istip:
                    node.label = get_name(
                        node.label)  # output multi-labeled tree for phyparts
            with open(phypartsDIR + treefile + "." + str(inclade_count),
                      "w") as outfile:
                outfile.write(newick3.tostring(inclade) + ";\n")
