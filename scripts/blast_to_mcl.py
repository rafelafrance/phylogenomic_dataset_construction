...
just arbitrarily pick a really small value to avoid negative values
...
def blast_to_mcl(
        blast_output, hit_frac_cutoff, exclude=EXCLUDE,
    ...
    outname = blast_output + ".hit-frac" + str(
        hit_frac_cutoff) + ".minusLogEvalue"
    if len(EXCLUDE) > 0:
        outname += ".exclude"
    ...
    for line in infile:
        ...
        if pident == 100.0 and query_taxon != hit_taxon:
            if abs(qend - qstart) / qlen >= 0.9 or abs(
                    send - sstart) / slen >= 0.9:
        ...
        if last_query == "":
            # The highest -log(evalue) for each query-hit pair that is not a
            # self hit
            ...
            if query == last_query and hit == last_hit:
                ...
                # all the query and hit are in the same direction for aa and
                # CDS
                ...
                if perc_qrange >= float(
                        hit_frac_cutoff) and perc_srange >= float(hit_frac_cutoff):
                    # output info for the previous query-hit pair
                    outfile.write(last_query + "\t" + last_hit + "\t" + str(
                        max_minusLogEvalue) + "\n")
                    count += 1
                    if count % 1000000 == 0:
                        print(str(count / 1000000),
                              "million hits written to the output")
                max_minusLogEvalue = minusLogEvalue  # reset
        # max_minusLogEvalue for a new query-hit pair
        last_query, last_qlen, last_qstart, last_qend = query, qlen, qstart, \
            qend
    ...
    perc_qrange = (last_qend - last_qstart + 1) / last_qlen
    perc_srange = (last_send - last_sstart + 1) / last_slen
    if perc_qrange >= float(hit_frac_cutoff) and perc_srange >= float(
            hit_frac_cutoff):
        outfile.write(last_query + "\t" + last_hit + "\t" + str(
            max_minusLogEvalue) + "\n")
    infile.close()
    ...
    print("Highly similar interspecific hits written to",
          blast_output + ".ident")
