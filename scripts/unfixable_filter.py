...
It writes a log with the number of corrected and unfix reads that are keept
and removed.
...
def grouper(iterable, n, fillvalue=None):  # Break data into fixed-length
    # chunks or blocks to parse fq in 4 lines at a time
...
def filter_unfix_pe(pe_fq1, pe_fq2, DIR):
    ...
    # path_pe_1, file_pe_1 = (os.path.split(pe_fq1)) #splits the path from
    # the file name
    ...
    if os.path.exists(DIR + filtered_1) and os.path.exists(DIR + filtered_2):
        ...
        with fqin1 as f1, fqin2 as f2:
            ...
            for entry in R1:
                ...
                head2, seq2, placeholder2, qual2 = [j.strip() for j in
                                                    next(R2)]
                ...
                if 'unfixable' in head1 or 'unfixable' in head2:
                    ...
                    head1 = head1.split('l:')[0][
                        :-1]  # keeps label information before the
                    # Rcorrector flags (low kmer stat, 'cor' and 'unfixable
                    # error')
        ...
        unfix_log.write(
            ...
            'reads:%s\nR1 corrected:%s\nR2 corrected:%s\npairs '
            'corrected:%s\n' % (
                counter, unfix_count, counter - unfix_count, r1_cor_count,
                r2_cor_count, pair_cor_count))
        ...
        with sein as se:
            ...
            for entry in SE:
                ...
                if 'unfixable' in head:
                    ...
                    head = head.split('l:')[0][
                        :-1]  # keeps label information before the Rcorrector flags (low kmer stat, 'cor' and 'unfixable error')
                    seout.write(
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        ...
        filter_unfix_pe(pe_fq1=sys.argv[1], pe_fq2=sys.argv[2],
                        DIR=sys.argv[3])
    else:
        print("Usage:")
        print(
            "For single end reads: python unfixable_filter.py fastq_se_reads output_dir")
        print(
            "For paired end reads: python unfixable_filter.py fastq_pe_reads1 fastq_pe_reads2 output_dir")
        sys.exit(0)
