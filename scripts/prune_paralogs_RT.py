...
def RT(homoDIR, tree_file_eneding, outDIR, min_ingroup_taxa,
    ...
    if len(set(INGROUPS) & set(OUTGROUPS)) > 0:
        print("Taxon ID", set(INGROUPS) & set(OUTGROUPS),
              "in both ingroups and outgroups")
        sys.exit(0)
    print(len(INGROUPS), "ingroup taxa and", len(OUTGROUPS),
          "outgroup taxa read")
    ...
    for treefile in os.listdir(homoDIR):
        ...
        if len(outgroup_names) > 0:  # at least one outgroup present, root
            # and cut inclades
            inclades = tree_utils.extract_rooted_ingroup_clades(
                curroot, INGROUPS, OUTGROUPS, min_ingroup_taxa)
            inclade_count = 0
            for inclade in inclades:
                ...
                for ortho in orthologs:
                    if len(tree_utils.get_front_labels(
                        ...
                        ortho_count += 1
                        with open(inclade_name + ".ortho" + str(
                                ortho_count) + ".tre", "w") as outfile:
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print(
            "python prune_paralogs_RT.py homoTreeDIR tree_file_eneding outDIR min_ingroup_taxa taxon_code_file")
        sys.exit(0)

    homoTreeDIR, tree_file_ending, outDIR, min_ingroup_taxa, taxon_code_file = sys.argv[
        1:]
    RT(homoTreeDIR, tree_file_ending, outDIR, min_ingroup_taxa,
       taxon_code_file)
