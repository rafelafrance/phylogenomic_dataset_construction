...
if __name__ == "__main__":
    ...
    cmd = "/home/yangya/dmorales/apps/ncbi-blast-2.7.1+/bin/makeblastdb -in "
    cmd += DIR + "all.fa -parse_seqids -dbtype prot -out " + DIR + "all.fa"
    print(cmd)
    os.system(cmd)

    # blast against the database one by one
    # setting the max_target_seqs to 1000 to avoid braking up large gene
    # families in MCL setting the evlalue cutoff to 10 to get as many hits
    # as possible
    for i in os.listdir(DIR):
        if i.endswith(FILE_ENDING) and i[:3] != "all":
            print("conducting blast on " + i)
            cmd = "/home/yangya/dmorales/apps/ncbi-blast-2.7.1+/bin/blastp "
            cmd += "-db " + DIR + "all.fa -query " + DIR + i
            cmd += " -evalue 10 -num_threads " + NUM_CORES
            cmd += " -max_target_seqs 1000 -out " + DIR + i + ".rawblastp "
            cmd += "-outfmt '6 qseqid qlen sseqid slen frames pident nident "
            cmd += "length mismatch gapopen qstart qend sstart send "
            cmd += "evalue bitscore'"
            print(cmd)
            os.system(cmd)
    print("blast is done")
