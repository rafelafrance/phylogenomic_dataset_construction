        ...
        for node in curroot.iternodes():  # walk through nodes
            ...
            for sister in node.get_sisters():
                if sister.istip and get_name(node.label) == get_name(
                    ...
                    # print node.label,unamb_chrDICT[node.label],
                    # sister.label,unamb_chrDICT[sister.label]
...
def main(treDIR, clnDIR, para, intree_file_ending=INTREE_FILE_ENDING):
    ...
    for i in os.listdir(treDIR):
        if i.endswith(intree_file_ending):
            ...
            if mask_para:
                curroot = mask_paraphyletic_tips(curroot, chrDICT)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(
            "python mask_tips_by_taxonID_transcripts.py treDIR aln-clnDIR mask_paraphyletic(y/n)")
        sys.exit(0)
