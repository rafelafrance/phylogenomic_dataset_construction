...
TRIMMOMATIC_CMD = 'java -jar ' + APPS_HOME + \
    "/Trimmomatic-0.36/trimmomatic-0.36.jar"  # Basic command
# to call Trimmomatic
TruSeq_ADAPTER = os.path.expanduser(
    "~/Desktop/botany_2018/databases/TruSeq_adapters.fa")  # User provided
# adapters


def trimmomatic_pe(pe_fq1, pe_fq2, num_cores, DIR):
    assert os.path.exists(
        TruSeq_ADAPTER), "Cannot fine the adapter file " + TruSeq_ADAPTER
...
if __name__ == "__main__":
    if len(sys.argv) == 4:
        trimmomatic_se(se_fq=sys.argv[1], num_cores=int(sys.argv[2]),
                       DIR=sys.argv[3])
    elif len(sys.argv) == 5:
        trimmomatic_pe(pe_fq1=sys.argv[1], pe_fq2=sys.argv[2],
                       num_cores=int(sys.argv[3]), DIR=sys.argv[4])
    else:
        print("Usage:")
        print(
            "For single end reads: python trimmomatic_wrapper.py fastq_se_reads num_cores output_dir ")
        print(
            "For paired end reads: python trimmomatic_wrapper.py fastq_pe_reads1 fastq_pe_reads2 num_cores output_dir")
        sys.exit(0)
