"""
Runs salmon or bowtie to remap read to transcritome assembly from Trinity and
cluster transcripts using Corset Salmon, bowtie and Corset must be in the path
"""
...
def salmon_index(transcript, num_cores, DIR):
    ...
    path_transcript, file_transcript = os.path.split(
        transcript)  # splits the path from the file name
    ...
    if os.path.exists(DIR + index_name):
        ...
        cmd = ["salmon-0.9.1", "index", "-t", transcript, "-i",
               DIR + index_name, "--type quasi", "-p", str(num_cores)]
        print((" ".join(cmd)))
        os.system(" ".join(cmd))
...
def salmon_quantify_pe(transcript, index, pe_fq1, pe_fq2, num_cores, DIR):
    ...
    if os.path.exists(DIR + quant_name):
        ...
        cmd = ["salmon-0.9.1", "quant", "-i", DIR + index_name, "--dumpEq",
               "--libType A", "-p", str(num_cores),
...
def salmon_quantify_se(transcript, index, se_fq, num_cores, DIR):
    ...
    # splits the path from the file name
...
def corset_salmon_eq_classes(transcript, eq_classes, DIR):
    ...
    if os.path.exists(DIR + clusters_name) and os.path.exists(
            DIR + counts_name):
        print("Found corset-salmon files for", salmon_base_name[0])
    else:
        cmd = ["corset", "-i salmon_eq_classes", eq_classes, "-m 5",
               "-p", (DIR + str(salmon_base_name[0]) + "_salmon")]
        print((" ".join(cmd)))
        os.system(" ".join(cmd))

    assert os.path.exists(
        DIR + clusters_name) and os.path.exists(
        DIR + counts_name), "Corsert did not finish"
...
def run_pe_salmon(transcript, pe_fq1, pe_fq2, num_cores, DIR):
    ...
    salmon_quantify_pe(transcript, DIR + index_name,
                       pe_fq1, pe_fq2, num_cores, DIR)
    corset_salmon_eq_classes(transcript, str(
        DIR + quant_name) + "/aux_info/eq_classes.txt", DIR)
...
def bowtie_index(transcript, num_cores, DIR):
    ...
    if os.path.isabs(transcript) == False:
        transcript = os.path.abspath(transcript)
    path_transcript, file_transcript = os.path.split(
        transcript)  # splits the path from the file name
    if path_transcript[-1] != "/":
        path_transcript += "/"
    ...
    if os.path.exists(DIR + index_1_bt) \
        ...
        cmd = ["bowtie-build", "-q", transcript,
               index_name, "--threads", str(num_cores)]
        print((" ".join(cmd)))
        os.system(" ".join(cmd))
        ...
        assert os.path.exists(DIR + index_1_bt) \
            ...
            and os.path.exists(DIR + index_rev_1_bt) \
            and os.path.exists(
            DIR + index_rev_2_bt), "bowtie-build not completed"
...
def bowtie_align_pe(transcript, index, pe_fq1, pe_fq2, num_cores, DIR):
    ...
    if os.path.exists(DIR + bam_file):
        ...
        cmd = ["bowtie", "--all", "-k 10", "--chunkmbs 2580", "--rf --nofw",
               "--threads", str(num_cores), (DIR + str(bowtie_base_name[0])),
...
def bowtie_align_se(transcript, index, se_fq, num_cores, DIR):
    ...
    if os.path.exists(DIR + bam_file):
        ...
        cmd = ["bowtie", "--all", "-k 10", "--threads", str(num_cores),
               DIR + str(bowtie_base_name[0]),
...
def corset_bowtie(transcript, bam, DIR):
    ...
    if os.path.exists(DIR + clusters_name) and os.path.exists(
        ...
        cmd = ["corset", "-i bam", bam, "-m -5", "-p",
               (DIR + str(bowtie_base_name[0]) + "_bowtie")]
        print((" ".join(cmd)))
        os.system(" ".join(cmd))
...
if __name__ == "__main__":
    if len(sys.argv) == 6 and sys.argv[5] == "salmon":
        run_se_salmon(transcript=sys.argv[1], se_fq=sys.argv[2], num_cores=int(
            sys.argv[3]), DIR=sys.argv[4])
    elif len(sys.argv) == 7 and sys.argv[6] == "salmon":
        run_pe_salmon(transcript=sys.argv[1], pe_fq1=sys.argv[2],
                      pe_fq2=sys.argv[3], num_cores=int(
            sys.argv[4]), DIR=sys.argv[5])
    elif len(sys.argv) == 6 and sys.argv[5] == "bowtie":
        run_se_bowtie(transcript=sys.argv[1], se_fq=sys.argv[2], num_cores=int(
            sys.argv[3]), DIR=sys.argv[4])
    elif len(sys.argv) == 7 and sys.argv[6] == "bowtie":
        run_pe_bowtie(transcript=sys.argv[1], pe_fq1=sys.argv[2],
                      pe_fq2=sys.argv[3], num_cores=int(
            sys.argv[4]), DIR=sys.argv[5])
    else:
        print("Usage:")
        print(
            "For single end reads: python corset_wrapper.py transcript "
            "fastq_se_reads num_cores output_dir aligner[salmon or bowtie]")
        print(
            "For paired end reads: python corset_wrapper.py transcript fastq_pe_reads1 fastq_pe_reads2 num_cores output_dir aligner[salmon or bowtie]")
        sys.exit(0)
