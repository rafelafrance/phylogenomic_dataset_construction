...
def jk(indir, num_core, resample_num, seqtype, replicates=200):
    ...
    with open(model_file, "r") as infile:
        for line in infile:
            if len(line) < 3:
                continue
            # line looks like DNA,
                # cluster2772rr.1to1ortho.aln-cln_gene22=23371-24283
    ...
    for i in range(replicates):
        ...
        try:  # remove intermediate files
            os.rename("RAxML_result." + repid,
                      outdir + "RAxML_result." + repid)
    ...
    os.system("cat " + outdir + "RAxML_result.* >" + str(
        resample_num) + "genes_trees")
    outfile1.close()
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print(
            "usage: python jackknife_by_number_genes.py indir num_core num_genes_to_resample dna/aa")
        sys.exit()
