            ...
            for sister in node.get_sisters():
                if sister.istip and get_name(node.label) == get_name(
                    ...
                    if len(curroot.leaves()) >= 4:
                        if node.nchildren == 1 or (
                                node == curroot and node.nchildren == 2):
    ...
    for i in os.listdir(DIR):
        if i[-3:] == ".tt":  # only mask trees that have tips trimmed
            ...
            with open(DIR + i + ".mm", "w") as outfile:
                outfile.write(
                    newick3.tostring(monophyly_masking_by_bl(intree)) + ";\n")

    if filecount == 0:
        print(
            "No file name with 'best' or 'tt' or 'fasttree' found in the treDIR")
