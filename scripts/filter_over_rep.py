...
def grouper(iterable, fillvalue=None):  # Break data into fixed-length chunks
    # or blocks to parse fq in 4 lines at a time
    args = [iter(iterable)] * 4
    return zip_longest(fillvalue=fillvalue, *args)


def ParseFastqcLog(fastqclog):  # Parse the fastqc_data.txt, find and
    # returns the over-represented sequences
    with open(fastqclog) as fp:
        for result in re.findall('Overrepresented sequences(.*?)END_MODULE',
                                 fp.read(), re.S):
    ...
    if os.path.exists(DIR + filtered_1) and os.path.exists(DIR + filtered_2):
        ...
        with fqin1 as f1, fqin2 as f2:
            ...
            for entry in R1:
                ...
                flagleft, flagright = seqsmatch(leftseqs, seq1), seqsmatch(
                    rightseqs, seq2)
        ...
        assert os.path.exists(DIR + filtered_1) \
            and os.path.exists(
            DIR + filtered_2), "Over-represented filtering not completed"
...
def filter_overep_se(se_fq, se_fqc, DIR):
    ...
    assert os.path.exists(
        DIR + filtered), "Over-represented filtering not completed"


if __name__ == "__main__":
    if len(sys.argv) == 4:
        filter_overep_se(se_fq=sys.argv[1], se_fqc=sys.argv[2],
                         DIR=sys.argv[3])
    elif len(sys.argv) == 6:
        filter_overep_pe(pe_fq1=sys.argv[1], pe_fq2=sys.argv[2],
                         pe_fqc1=sys.argv[3], pe_fqc2=sys.argv[4],
                         DIR=sys.argv[5])
    else:
        print("Usage:")
        print(
            "For single end reads: python over_represented_filter.py fastq_se_reads fastQC_data_se output_dir")
        print(
            "For paired end reads: python over_represented_filter.py fastq_pe_reads1 fastq_pe_reads2 fastQC_data1 fastQC_data2 output_dir")
        sys.exit(0)
