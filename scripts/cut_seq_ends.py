...
def cut_seq_ends(fasta, blast_output, logfile="log"):
    ...
    with open(blast_output, "r") as infile:
        for line in infile:
            ...
            cutDICT[query][0] = min(cutDICT[query][0], qstart,
                                    qend)  # compare starts
            cutDICT[query][1] = max(cutDICT[query][1], qstart,
                                    qend)  # compare ends
            cutDICT[hit][0] = min(cutDICT[hit][0], sstart,
                                  send)  # compare starts
            cutDICT[hit][1] = max(cutDICT[hit][1], sstart,
                                  send)  # compare ends
    ...
    with open(fasta, "r") as handle:
        for record in SeqIO.parse(handle, "fasta"):
            ...
            if seqid in cutDICT:
                ...
                pass  # remove seqs with no interspecific hits
    outfile.close()
    summary = "taxonid\tbefore\thalf_left\tafter\tperc_survive\n"
    for i in outdict:
        out = outdict[i]
        summary += i + "\t" + str(out[0]) + "\t" + str(out[1]) + "\t" + str(
            out[2]) + "\t"
