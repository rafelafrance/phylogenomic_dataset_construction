...
RCORRECTOR_CMD = 'perl ' + APPS_HOME + "/Rcorrector/run_rcorrector.pl"  #
# Basic command to call rcorrector
...
def rcorrector_pe(pe_fq1, pe_fq2, num_cores, DIR):
    ...
    if (os.path.splitext(file_pe_1)[-1]) and (
        ...
        corrected_1 = (base_name_1[0]) + ".cor.fq"  # input paired fq1 with
        # extension ".cor.fq"
        corrected_2 = (base_name_2[
    ...
    if os.path.exists(DIR + corrected_1) and os.path.exists(DIR + corrected_2):
        ...
        cmd = [RCORRECTOR_CMD, "-1", pe_fq1, "-2", pe_fq2, "-t",
               str(num_cores), "-od", DIR]
        print((" ".join(cmd)))
        os.system(" ".join(cmd))
...
if __name__ == "__main__":
    if len(sys.argv) == 4:
        rcorrector_se(se_fq=sys.argv[1], num_cores=int(sys.argv[2]),
                      DIR=sys.argv[3])
    elif len(sys.argv) == 5:
        rcorrector_pe(pe_fq1=sys.argv[1], pe_fq2=sys.argv[2],
                      num_cores=int(sys.argv[3]), DIR=sys.argv[4])
    else:
        print("Usage:")
        print(
            "For single end reads: python rcorrector.py fastq_se_reads num_cores output_dir")
        print(
            "For paired end reads: python rcorrector.py fastq_pe_reads1 num_cores fastq_pe_reads2 output_dir")
        sys.exit(0)
