...
if __name__ == "__main__":
    ...
    for i in os.listdir(DIR):
        ...
        for line in infile:
            if line[0] != ">":
                continue  # only look at seq names
            taxon, seqID = (line.strip()[1:]).split("@")
            assert taxon == taxonID, taxon + " taxon id do not match file " \
                "name " + taxonID
            assert match("^[a-zA-Z0-9_]+$",
                         seqID), seqID + " seq id contains special " \
                "characters other than letter, digit " \
                "and '_'"
            seqIDs.append(seqID)
