    ...
    for i in seqs_sample:
        ...
        for j in range(len(seqs_sample[i])):
            for k in range(len(seqs_sample[i])):
                if j < k:
                    ovr = check_overlap(seqs_sample[i][j].seq,
                                        seqs_sample[i][k].seq)
                    if ovr == True:
                        # print "\t",ovr,j,k,seqs_sample[i][j].name,
                        # seqs_sample[i][k].name
                        G.add_edge(seqs_sample[i][j].name,
                                   seqs_sample[i][k].name)
        if len(G) > 0:
            ...
            if len(j) == 2:
                ...
                while going:
                    ...
                    for k in j.edges():
                        ...
                        finalse = combine_seqs(seqs_d[ks[0]].seq,
                                               seqs_d[ks[1]].seq)
                        ns = seq.Sequence()
