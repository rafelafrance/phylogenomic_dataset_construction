...
def fastQC_pe(pe_fq1, pe_fq2, num_cores, DIR):
    ...
    if os.path.exists(DIR + fqc_html_1) and os.path.exists(DIR + fqc_html_1):
        ...
        cmd = ["fastqc", pe_fq1, pe_fq2, "-t", str(num_cores), "-o", DIR,
               "--extract"]
        print((" ".join(cmd)))
        os.system(" ".join(cmd))
...
if __name__ == "__main__":
    if len(sys.argv) == 4:
        fastQC_se(se_fq=sys.argv[1], num_cores=int(sys.argv[2]),
                  DIR=sys.argv[3])
    elif len(sys.argv) == 5:
        fastQC_pe(pe_fq1=sys.argv[1], pe_fq2=sys.argv[2],
                  num_cores=int(sys.argv[3]), DIR=sys.argv[4])
    else:
        print("Usage:")
        print(
            "For single end reads: python fastqc_wrapper.py fastq_se_reads num_cores output_dir")
        print(
            "For paired end reads: python fastqc_wrapper.py fastq_pe_reads1 fastq_pe_reads2 num_cores output_dir")
        sys.exit(0)
