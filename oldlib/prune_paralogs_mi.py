...
def prune_mi(score_tuple, node, root, pp_trees):
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print(
            "python prune_paralogs_MI.py homoTreeDIR tree_file_ending "
            "relative_tip_cutoff absolute_tip_cutoff MIN_TAXA outDIR")
        print(
            "LONG_TIP_CUTOFF is typically same value of the previous "
            "LONG_TIP_CUTOFF")
        sys.exit(0)

    inDIR = sys.argv[1] + "/"
    tree_file_ending = sys.argv[2]
    relative_tip_cutoff, absolute_tip_cutoff = float(sys.argv[3]), float(
        sys.argv[4])
        ...
        if get_front_score(curroot) >= MIN_TAXA:  # No need to prune
            ...
            while going:  # python version of do..while loop
                ...
                if highest >= MIN_TAXA:  # prune
                    curroot, done = prune_mi(score_hashes[highest_node],
                                             highest_node, curroot, pp_trees)
